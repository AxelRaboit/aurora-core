...
- [pitfall_async_handler_error_boundary.md](pitfall_async_handler_error_boundary.md) - un gestionnaire `async` branché nu sur `@click` reçoit l'événement à la place de son argument ; la promesse rejetée remonte jusqu'au `errorCaptured` de la page, qui vide l'écran entier. Appeler avec ses arguments, et ignorer un argument du mauvais type dans le composable.
